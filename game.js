...
let BW = 1200, BH = 600;
let SCALE = 1.0;
let MOBILE_SCALE = 1.0;   // faktor tambahan khusus HP
...
// === UTILITY ===
// sc() sekarang memperhitungkan MOBILE_SCALE juga
...
function layout() {
  ...
  // Deteksi mobile: layar sempit atau touch device
  IS_MOBILE = WIDTH < 800 || ('ontouchstart' in window && WIDTH < 1100);

  // MOBILE_SCALE: perbesar UI di HP
  // Di desktop = 1.0, di HP bisa 1.35 - 1.6
  if (IS_MOBILE) {
    MOBILE_SCALE = Math.min(WIDTH / 480, 1.6);   // makin sempit, makin besar
  ...
  // === JOYSTICK DIPERBESAR ===
  // Basis radius dinaikkan dari 55 → 80, lalu dikali MOBILE_SCALE
...
// === FONT ===
// Font di HP diperbesar otomatis karena sc() sudah termasuk MOBILE_SCALE
...
// === JOYSTICK (diperbesar & tanpa panah) ===
function gambar_joystick() {
  joy_visible = true;
  const alpha_mul = (joy_pressed || joy_dragging) ? 1.0 : 0.45;
  const R = JOY_FIXED_RADIUS;
  const kr = Math.floor(R * 0.45);   // knob proporsional dengan radius

  const a_fill = Math.floor((joy_pressed ? 80 : 50) * alpha_mul);
  const a_line = Math.floor((joy_pressed ? 180 : 110) * alpha_mul);
  ...
  // Garis silang tipis di tengah (opsional, biar keliatan arah)
  ctx.lineWidth = sc(2);
  ctx.strokeStyle = `rgba(255,255,255,${a_line/255 * 0.5})`;
  ctx.beginPath();
  ctx.moveTo(joy_c.x, joy_c.y - R*0.7); ctx.lineTo(joy_c.x, joy_c.y + R*0.7); ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(joy_c.x - R*0.7, joy_c.y); ctx.lineTo(joy_c.x + R*0.7, joy_c.y); ctx.stroke();
  ...
  if (joy_pressed && joy_dir) {
    ...
    grad.addColorStop(0, `rgba(${gc[0]},${gc[1]},${gc[2]},0.5)`);
  ...
  // Knob (tuas)
  const ka = (joy_pressed ? 250 : 220) * alpha_mul;
...
function top_bar(f) {
  ...
  // tombol exit diperbesar di mobile
...
// === GAME OVER (panel & tombol diperbesar di mobile) ===
function layar_game_over(skor, panjang, fps) {
  return new Promise(resolve => {
    ...
    function frame(now) {
      ...
      // Card skor & best diperbesar di mobile
