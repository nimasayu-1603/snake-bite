...
// === UTILITY ===
function sc(n) { return Math.max(1, Math.floor(n * SCALE * MOBILE_SCALE)); }
...
function layout() {
  ...
  IS_MOBILE = WIDTH < 800 || ('ontouchstart' in window && WIDTH < 1100);

  if (IS_MOBILE) {
    MOBILE_SCALE = Math.min(WIDTH / 480, 1.6);
    MOBILE_SCALE = Math.max(1.25, MOBILE_SCALE);
  } else {
    MOBILE_SCALE = 1.0;
  }
  ...
  const baseR = IS_MOBILE ? 80 : 55;
  JOY_FIXED_RADIUS = sc(baseR);

  const JOY_MARGIN_X = sc(20);
  const JOY_MARGIN_Y = sc(100);
  joy_c = {
...
// === JOYSTICK dengan warna MINT & PINK ===
function gambar_joystick() {
  joy_visible = true;
  const alpha_mul = (joy_pressed || joy_dragging) ? 1.0 : 0.55;
  const R = JOY_FIXED_RADIUS;
  const kr = Math.floor(R * 0.42);   // knob lebih kecil dari base

  // === WARNA BASE (LUAR) - MINT ===
  const baseColor = MINT;
  const baseFillAlpha = (joy_pressed ? 110 : 70) * alpha_mul;
  const baseLineAlpha = (joy_pressed ? 230 : 170) * alpha_mul;

  // isi base MINT transparan
  ctx.fillStyle = `rgba(${baseColor[0]},${baseColor[1]},${baseColor[2]},${baseFillAlpha/255})`;
  ctx.beginPath(); ctx.arc(joy_c.x, joy_c.y, R, 0, Math.PI*2); ctx.fill();

  // outline base MINT lebih tebal
  ctx.strokeStyle = `rgba(${baseColor[0]},${baseColor[1]},${baseColor[2]},${baseLineAlpha/255})`;
  ctx.lineWidth = sc(5);
  ctx.beginPath(); ctx.arc(joy_c.x, joy_c.y, R, 0, Math.PI*2); ctx.stroke();

  // === WARNA KNOB (DALAM) - PINK, berubah sesuai arah ===
  let knobColor = PINK;   // default PINK
  if (joy_pressed && joy_dir) {
    if (joy_dir === ATAS)  knobColor = MINT;
    else if (joy_dir === BAWAH) knobColor = PINK;
    else if (joy_dir === KIRI)  knobColor = LAV;
    else if (joy_dir === KANAN) knobColor = PEACH;
  }

  const knobAlpha = (joy_pressed ? 255 : 230) * alpha_mul;

  // glow lembut di sekeliling knob
  const grad = ctx.createRadialGradient(joy_p.x, joy_p.y, 0, joy_p.x, joy_p.y, kr*1.8);
  grad.addColorStop(0, `rgba(${knobColor[0]},${knobColor[1]},${knobColor[2]},${0.55 * alpha_mul})`);
  grad.addColorStop(1, `rgba(${knobColor[0]},${knobColor[1]},${knobColor[2]},0)`);
  ctx.fillStyle = grad;
  ctx.beginPath(); ctx.arc(joy_p.x, joy_p.y, kr*1.8, 0, Math.PI*2); ctx.fill();

  // isi knob
  ctx.fillStyle = `rgba(${knobColor[0]},${knobColor[1]},${knobColor[2]},${knobAlpha/255})`;
  ctx.beginPath(); ctx.arc(joy_p.x, joy_p.y, kr, 0, Math.PI*2); ctx.fill();

  // outline knob (cream putih)
  ctx.strokeStyle = `rgba(252,244,228,${alpha_mul})`;
  ctx.lineWidth = sc(3);
  ctx.beginPath(); ctx.arc(joy_p.x, joy_p.y, kr, 0, Math.PI*2); ctx.stroke();

  // titik highlight kecil di knob
  ctx.fillStyle = `rgba(255,255,255,${0.6 * alpha_mul})`;
  ctx.beginPath();
  ctx.arc(joy_p.x - kr*0.3, joy_p.y - kr*0.3, Math.max(1, Math.floor(kr*0.18)), 0, Math.PI*2);
  ctx.fill();
}

function joystick_hit(mx, my) {
  const R = JOY_FIXED_RADIUS + sc(30);
...
function top_bar(f) {
  ...
  const f_title = font(IS_MOBILE ? 16 : 20);
  ctx.font = f_title;
  ctx.textBaseline = "middle";
  const pasangan = [
  ...
  ctx.font = font(IS_MOBILE ? 11 : 13, false);
  ...
  const ebw = sc(IS_MOBILE ? 46 : 34);
  const ebh = sc(IS_MOBILE ? 36 : 26);
...
function hud(f, skor, pjg, fps) {
  ...
  const pw = sc(IS_MOBILE ? 260 : 210);
  const ph = sc(IS_MOBILE ? 70 : 50);
  ...
  for (let i = 0; i < 3; i++) {
    ...
    const icx = px + sc(IS_MOBILE ? 34 : 26), icy = py + ph/2;
    const iconSize = IS_MOBILE ? 1.35 : 1;

    if (ico === "speed") {
      const ri = sc(11 * iconSize);
      ctx.strokeStyle = rgb(CREAM); ctx.lineWidth = sc(2);
      ctx.beginPath();
      ctx.arc(icx, icy, ri, Math.PI*0.75, Math.PI*2.25);
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(icx, icy+sc(2)); ctx.lineTo(icx+sc(6*iconSize), icy-sc(4*iconSize)); ctx.stroke();
      ctx.fillStyle = rgb(CREAM);
      ctx.beginPath(); ctx.arc(icx, icy+sc(2), sc(2), 0, Math.PI*2); ctx.fill();
    } else if (ico === "length") {
      const r = sc(5 * iconSize);
      ctx.fillStyle = rgb(CREAM);
      ctx.beginPath(); ctx.arc(icx-sc(6*iconSize), icy, r, 0, Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.arc(icx+sc(3*iconSize), icy, r, 0, Math.PI*2); ctx.fill();
      ctx.fillStyle = rgb([26,24,30]);
      ctx.beginPath(); ctx.arc(icx-sc(8*iconSize), icy-sc(2), sc(1), 0, Math.PI*2); ctx.fill();
      ctx.beginPath(); ctx.arc(icx-sc(4*iconSize), icy-sc(2), sc(1), 0, Math.PI*2); ctx.fill();
    } else {
      const r = sc(10 * iconSize);
      ctx.fillStyle = rgb(PINK);
      ctx.beginPath(); ctx.arc(icx, icy, r, 0, Math.PI*2); ctx.fill();
      ctx.strokeStyle = rgb([80,60,60]); ctx.lineWidth = sc(2);
      ctx.beginPath();
      ctx.moveTo(icx, icy-r+sc(1)); ctx.lineTo(icx+sc(1), icy-r-sc(4)); ctx.stroke();
      ctx.fillStyle = rgb(MINT);
      ctx.beginPath(); ctx.ellipse(icx+sc(1), icy-r-sc(3), sc(3), sc(2), 0, 0, Math.PI*2); ctx.fill();
      ctx.fillStyle = "#fff";
      ctx.beginPath(); ctx.arc(icx-sc(3), icy-sc(3), sc(2), 0, Math.PI*2); ctx.fill();
    }

    const text_x = px + sc(IS_MOBILE ? 70 : 52);

    ctx.font = f.lbl;
    ctx.fillStyle = rgb(TEXT_MID);
    ctx.textAlign = "left";
    ctx.textBaseline = "top";
    ctx.fillText(lbl, text_x, py + sc(10));

    ctx.font = f.val;
    ctx.fillStyle = rgb(CREAM);
    ctx.textBaseline = "top";
    ctx.fillText(String(val), text_x, py + sc(IS_MOBILE ? 32 : 22));
  }
}
...
function layar_start() {
  return new Promise(resolve => {
    ...
    function frame(now) {
      ...
      const bpw = sc(IS_MOBILE ? 320 : 260);
      const bph = sc(IS_MOBILE ? 80 : 60);
      const bp = {x: cx-bpw/2, y: y_p-bph/2, w: bpw, h: bph};
      ...
      const tri = sc(IS_MOBILE ? 18 : 14);
      const tcx = bp.x + bp.w/2 - sc(IS_MOBILE ? 55 : 42), tcy = bp.y + bp.h/2;
      ctx.fillStyle = rgb([26,24,30]);
      ctx.beginPath();
      ...
      ctx.textAlign = "center"; ctx.textBaseline = "middle";
      ctx.fillText("Play", bp.x + bp.w/2 + sc(IS_MOBILE ? 25 : 18), bp.y + bp.h/2);
...
function layar_game_over(skor, panjang, fps) {
  return new Promise(resolve => {
    ...
    function draw_tombol(r, txt, clr, hover, ico, f) {
      ...
      let off = 0;
      if (ico === "r") {
        const icx = r.x + sc(IS_MOBILE ? 42 : 32), icy = r.y + r.h/2, ri = sc(IS_MOBILE ? 15 : 11);
        ...
        ctx.closePath(); ctx.fill();
        off = sc(IS_MOBILE ? 20 : 14);
      } else if (ico === "e") {
        const icx = r.x + sc(IS_MOBILE ? 42 : 32), icy = r.y + r.h/2, si = sc(IS_MOBILE ? 11 : 8);
        ctx.strokeStyle = rgb([26,24,30]); ctx.lineWidth = sc(3);
        ctx.beginPath(); ctx.moveTo(icx-si, icy-si); ctx.lineTo(icx-si, icy+si); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(icx-si, icy-si); ctx.lineTo(icx, icy-si); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(icx-si, icy+si); ctx.lineTo(icx, icy+si); ctx.stroke();
    ...
    function frame(now) {
      ...
      const pw = sc(IS_MOBILE ? 580 : 480);
      const ph = sc(IS_MOBILE ? 620 : 500);
      const panel = {x: cx - pw/2, y: cy - ph/2, w: pw, h: ph};
      const y_t = panel.y + sc(55);
      const y_u = panel.y + sc(140);
      const y_s = panel.y + sc(240);
      const y_btn = panel.y + sc(430);

      const btnW = sc(IS_MOBILE ? 220 : 180);
      const btnH = sc(IS_MOBILE ? 80 : 60);
      const gapBtn = sc(IS_MOBILE ? 40 : 20);
      const b1 = {x: panel.x + pw/2 - gapBtn/2 - btnW, y: y_btn - btnH/2, w: btnW, h: btnH};
      const b2 = {x: panel.x + pw/2 + gapBtn/2,       y: y_btn - btnH/2, w: btnW, h: btnH};
      ...
      const n = 5, ds = sc(IS_MOBILE ? 38 : 30), gap = sc(2);
      const tw = ds*n + gap*(n-1);
      ...
      const cw = sc(IS_MOBILE ? 240 : 185);
      const ch = sc(IS_MOBILE ? 120 : 92);
      ...
      buah(card1.x + sc(15), card1.y + sc(IS_MOBILE ? 32 : 24),
           sc(IS_MOBILE ? 42 : 32), sc(IS_MOBILE ? 42 : 32), BUAH[5], t);

      ctx.font = f.lbl; ctx.fillStyle = rgb(CREAM);
      ctx.textAlign = "left"; ctx.textBaseline = "top";
      ctx.fillText("Score", card1.x + sc(IS_MOBILE ? 75 : 62), card1.y + sc(IS_MOBILE ? 28 : 22));
      ctx.font = f.val;
      ctx.fillText(String(skor), card1.x + sc(IS_MOBILE ? 75 : 62), card1.y + sc(IS_MOBILE ? 60 : 44));
      ...
      const ccx = card2.x + sc(IS_MOBILE ? 48 : 38);
      const ccy = card2.y + sc(IS_MOBILE ? 55 : 42);
      const sc_ = sc(IS_MOBILE ? 16 : 12);
      ...
      ctx.font = f.lbl; ctx.fillStyle = rgb(CREAM);
      ctx.fillText("Best Score", card2.x + sc(IS_MOBILE ? 75 : 62), card2.y + sc(IS_MOBILE ? 28 : 22));
      ctx.font = f.val;
      ctx.fillText("37", card2.x + sc(IS_MOBILE ? 75 : 62), card2.y + sc(IS_MOBILE ? 60 : 44));
      ...
      ctx.textAlign = "center"; ctx.textBaseline = "top";
      ctx.fillText("(R)", b1.x + b1.w/2, b1.y + b1.h + sc(20));
      ctx.fillText("(Space)", b2.x + b2.w/2, b2.y + b2.h + sc(20));
